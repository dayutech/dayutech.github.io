...
线性回归模型$\beta$地估计量为$\hat{\beta}$
$$
\hat{\beta} = \beta + \mathbb{E}[\varepsilon |X]
$$
...
扰动项
$$
\varepsilon  = \textbf{Y} - (\beta_0 + \beta_1\textbf{X}_1 +\beta_2\textbf{X}_2+\beta_3\textbf{X}_3+...+\beta_4\textbf{X}_4) = \textbf{Y} - \mathbb{E}[\textbf{Y}|\textbf{X}]
$$
所以在给定$\textbf{X}$的情况下$\varepsilon$的条件期望为
$$
\begin{aligned}
\mathbb{E}[\varepsilon|\textbf{X}] &= \mathbb{E}[\textbf{Y}-(\beta_0 + \beta_1\textbf{X}_1 +\beta_2\textbf{X}_2+\beta_3\textbf{X}_3+...+\beta_4\textbf{X}_4)|\textbf{X}] \qquad\qquad \text{期望在给定X的时候为常数，向外提}\\\\
&=\mathbb{E}[\textbf{Y}|\textbf{X}] - (\beta_0 + \beta_1\textbf{X}_1 +\beta_2\textbf{X}_2+\beta_3\textbf{X}_3+...+\beta_4\textbf{X}_4)
\end{aligned}
$$
