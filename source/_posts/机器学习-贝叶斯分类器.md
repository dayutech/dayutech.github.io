...
# 朴素贝叶斯分类器
基于贝叶斯公式来估计后验概率$P(c\|x)$主要的困难在于类条件概率$P(x\|c)$的估计，难以从有限的训练样本中直接估计得出。为了避开这个障碍，朴素贝叶斯分类器采用了属性条件独立性假设。
对已知类别假设所有属性独立。基于这个假设将贝叶斯公式重写：  
$$
P(c\|x)=\frac{P(c)P(x\|c)}{P(x)} = \frac{P(c)}{P(x)} \prod_{i=1}^{d} P(x_i\|\theta_c) \tag{16}
$$
所以得到朴素贝叶斯分类器表达式  
$$
h\*(x)=\underset{c \in y}{arg \space max } \space P(c)\prod_{i=1}^{d} P(x_i\|\theta_c) \tag{17}
$$
关于$P(c)$的估计上一节介绍过了
关于$P(x_i\|c)$的估计对于连续参数就是上一节的极大似然发介绍的方法 ，对于离散参数则使用$D_c$中第i个属性的样本组成的集合，然后再将所有属性的估计结果相乘。
# 半朴素贝叶斯分类器
...
