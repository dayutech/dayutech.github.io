...
3. 尝试提取 GAV，回溯 release tag 以找到可用构建补充（找到一个可用的发布版本提取依赖从而完成变更编译）；
4. 仅为变更文件准备编译所需依赖并编译；
...
知识库侧通常维护两类倒排索引：
- FQN 索引 $I_{FQN}$：`构件 FQN -> {CVE, change, branch, 构件标识}`；
- 非限定索引 $I_{UQ}$：`构件非限定标识 -> {CVE, change, branch, 构件标识}`。
...
召回流程：
1. 默认模式（按构件 FQN）  
   从目标 JAR 提取全部构件的 FQN，在 $I_{FQN}$ 中查命中并并集化，得到首批候选 CVE。
2. 重打包 / 重定位模式（按构件非限定标识）  
   当发生 relocation 或 fat-jar 混装时，构件 FQN 常失真，再提取同一批构件的非限定标识，到 $I_{UQ}$ 中召回补充候选。
3. 候选合并  
...
```mermaid
flowchart TB
  IN["输入: 目标 JAR 中的构件集合<br/>包含 FQN 与非限定标识"]
  ...
  M["候选并集"]
  N["抑噪过滤<br/>语言一致性 / 最小证据 / 高频符号降权"]
  O["输出: 候选 CVE + 命中证据"]
  P["进入构件级与 triplet 级精判"]
...
退化情形需要额外处理：
- 若 $NT \neq \varnothing$ 且 $PT \neq \varnothing$，直接比较 $\lvert NT \cap T_m \rvert$ 与 $\lvert PT \cap T_m \rvert$ 即可。
- 若仅新增代码，则 $NT=\varnothing$，原判定式会退化为 `0 >= |PT ∩ T_m|`，因此需额外检查 $\lvert PT \cap T_m \rvert$ 是否达到阈值 $\theta_{PT}$。
- 若仅删除代码，等价地考虑有 $PT=\varnothing$，判定逻辑也应该对称退化，此时从逻辑完备性看，也应要求 $\lvert NT \cap T_m \rvert$ 达到删除侧阈值 $\theta_{NT}$ 后，才可判为更接近修复前，但一般仅删除不仅仅只是会导致$NT \neq \varnothing$，同时也会新增边此时 $PT \neq \varnothing$ 所以一般不特别考虑。
